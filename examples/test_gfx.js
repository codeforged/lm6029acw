...
if (!lcd.begin()) {
  console.error(
    "Gagal membuka /dev/spidev0.0. Pastikan SPI sudah di-enable (raspi-config).",
  );
  process.exit(1);
}
...
// Jalur printText() HARUS menghormati warna yang sama. Regresi 1.1.1:
// dulu printText() memaksa setTextColor(1), jadi baris ini tidak tampil
// (glyph digambar nyala di atas latar yang juga nyala).
lcd.printText("via printText()", 10, 40, 1);
